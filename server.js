...
// ─────────────────────────────────────────────────────────
// 카카오 OAuth 콜백 페이지  GET /oauth/kakao
//   팝업 창에서 열림 — 인가코드를 부모 창에 postMessage로 전달 후 닫힘
// ─────────────────────────────────────────────────────────
app.get('/oauth/kakao', (req, res) => {
  const code  = JSON.stringify(req.query.code  || '');
  const error = JSON.stringify(req.query.error || '');
  res.send(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>카카오 로그인</title></head><body>
<script>
  const code="${req.query.code||''}", error="${req.query.error||''}";
  if(code) window.opener?.postMessage({type:'KAKAO_AUTH_CODE',code},'*');
  else     window.opener?.postMessage({type:'KAKAO_AUTH_ERROR',error},'*');
  window.close();
</script></body></html>`);
});

// ─────────────────────────────────────────────────────────
// 카카오 로그인  POST /api/kakao-auth
//   인가코드(code) → 액세스토큰 교환 → 사용자 정보 → Firebase Custom Token
// ─────────────────────────────────────────────────────────
const KAKAO_REST_KEY = '7422b44d47cfb963938f8e381d83b58b';

app.post('/api/kakao-auth', async (req, res) => {
  const { code, redirectUri } = req.body;
  if (!code)        return res.status(400).json({ error: '인가코드 필요' });
  if (!_fbAuth)     return res.status(500).json({ error: 'Firebase 미초기화' });
  ...
  try {
    ...
    const ku       = kakaoRes.data;
    const uid      = `kakao_${ku.id}`;
    ...
    const userProps = { displayName: nickname, photoURL };
    if (email) userProps.email = email;
    try {
      await _fbAuth.updateUser(uid, userProps);
    } catch (e) {
      if (e.code === 'auth/user-not-found') {
        await _fbAuth.createUser({ uid, ...userProps });
      } else throw e;
    }

    // 4. Firebase Custom Token 발급
    const customToken = await _fbAuth.createCustomToken(uid, { provider: 'kakao' });
    console.log(`[kakao-auth] ✅ uid=${uid} email=${email || '없음'}`);

    res.json({ customToken, nickname, email, photoURL });
  } catch (err) {
    console.error('[kakao-auth] 오류:', err.message);
    res.status(500).json({ error: '카카오 인증 실패', detail: err.message });
  }
});
